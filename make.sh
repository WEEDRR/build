...
function info() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
        -d chat_id="$chat_id" \
        -d "parse_mode=markdown" \
        -d text="NeeSan EAS new build!%0AFor device X00TD (Asus Max Pro M1)%0AAt branch eas-test"
}
# Compile plox
function compile() {
...
function zipping() {
    ...
    if [ "$is_test" = true ]; then
        zip -r9 NeeSan-EAS-Test-"${TANGGAL}".zip * -x LICENCE README.md
    else
        zip -r9 NeeSan-EAS-"${TANGGAL}".zip * -x LICENCE README.md
    fi #ngentod
    cd .. #well
}
info
